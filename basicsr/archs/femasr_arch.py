...
class FeMaSRNet(nn.Module):
    ...
    def encode_and_decode(self, input, gt_indices=None, current_iter=None):
        ...
        loss_dict['semantic_loss'] = sum(semantic_loss_list) if len(semantic_loss_list) else codebook_loss * 0

        return out_img, loss_dict, indices_list
    ...
    def forward(self, input, gt_indices=None):
        ...
        if gt_indices is not None:
            ...
            dec, loss_dict, indices = self.encode_and_decode(input, gt_indices)
        else:
            # in HQ stage, or LQ test stage, no GT indices needed.
            dec, loss_dict, indices = self.encode_and_decode(input)

        return dec, loss_dict, indices
