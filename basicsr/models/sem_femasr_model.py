...
class SemFeMaSRModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.AdaCode_stage and not self.LQ_stage:
            # >>> [STAGE 2]
            load_path = self.opt['path'].get('pretrain_codebook', None)
            assert load_path is not None, 'Need to specify hq prior model path in codebook stage'
            self.load_codebook(self.net_g.quantize_group, load_path, False)
    ...
    def load_codebook(self, net, load_path, strict=True, param_key='params'):
        ...
        codebook_keys = [v for v in load_net.keys() if 'quantize_group.0' in v]
        print(codebook_keys)
        for i, codebook_key in enumerate(codebook_keys):
            self.net_g.quantize_group[i].state_dict()['embedding.weight'].copy_(load_net[f'quantize_group.0.{i}.embedding.weight'])


    def _get_codebook_config(self, load_path):
        codebook_params = []
        if self.AdaCode_stage and not self.LQ_stage:
    ...
    def feed_data(self, data):
        ...
        if self.SEM_stage:
            self.lq_sem = data['lq_sem'].to(self.device)
        if 'gt' in data:
            self.gt = data['gt'].to(self.device)
            if self.SEM_stage:
                self.gt_sem = data['gt_sem'].to(self.device)
    ...
    def optimize_parameters(self, current_iter, finetune_all=False):
        ...
        if self.LQ_stage and not finetune_all:
            ...
            with torch.no_grad():
                if self.SEM_stage:
                    self.gt_rec, _, gt_aux = self.net_hq(self.gt, self.gt_sem)
                else:
                    self.gt_rec, _, gt_aux = self.net_hq(self.gt)

            if self.SEM_stage:
                if train_opt.get('synfeatmatch', False):
                    self.output, losses, _ = self.net_g(self.lq, self.lq_sem, gt_aux, self.net_hq.module.multiscale_encoder)
                else:
                    self.output, losses, _ = self.net_g(self.lq, self.lq_sem, gt_aux)
            else:
                if train_opt.get('synfeatmatch', False):
            ...
            if self.SEM_stage:
                self.output, losses, _ = self.net_g(self.lq, self.lq_sem) 
            else:
            ...
            # stage 1 && stage 2.
            if self.SEM_stage:
                self.output, losses, _ = self.net_g(self.gt, self.gt_sem) 
            else:
    ...
    def test(self):
        ...
        lq_input = self.lq
        if self.SEM_stage:
            mask_input = self.lq_sem
            _, _, h, w = lq_input.shape
            if h*w < min_size:
                self.output = net_g.test(lq_input, mask_input)
            else:
                self.output = net_g.test_tile(lq_input, mask_input)
        else:
            _, _, h, w = lq_input.shape
            if h*w < min_size:
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        if with_metrics:
            ...
            for metric in self.metric_results.keys():
                ...
                self.metric_results[metric] = self.metric_results[metric] / len(dataloader)     
            ...
            if self.key_metric is not None:
                ...
                if to_update:
                    ...
                    # save_path = os.path.join(self.opt['path']['models'], 'my_net_g_best.pth')
                    # print(f'@@@ MY Saving best net_g to {save_path}')
