...
class PairedImageDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if opt.get('dataroot_gt') is not None:
            ...
            paths = make_dataset(self.gt_folder)
        ...
        if opt.get('dataroot_lq') is not None:
            ...
            paths = make_dataset(self.lq_folder)
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            if self.opt['use_resize_crop']:
                if gt_size > img_gt.shape[0] or gt_size > img_gt.shape[1]:
            ...
            if self.opt['use_no_random_resize_crop']:
                img_gt = cv2.resize(img_gt, (gt_size, gt_size), interpolation=cv2.INTER_CUBIC)
                img_lq = cv2.resize(img_lq, (gt_size, gt_size), interpolation=cv2.INTER_CUBIC)
        ...
        if self.opt['phase'] != 'train':
            random_crop_eval_size = self.opt.get('random_crop_eval_size', None)
            if random_crop_eval_size:
                input_gt_size = img_gt.shape[0]
                input_lq_size = img_lq.shape[0]
                scale = input_gt_size // input_lq_size
            ...
            if crop_eval_size:
                img_gt = cv2.resize(img_gt, (crop_eval_size, crop_eval_size), interpolation=cv2.INTER_CUBIC)
                img_lq = cv2.resize(img_lq, (crop_eval_size, crop_eval_size), interpolation=cv2.INTER_CUBIC)
